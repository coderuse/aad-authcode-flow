...
(function (Msal) {
    ...
    var UserAgentApplication = (function () {
        ...
        UserAgentApplication.prototype.loginPopup = function (promptValue, scopes, extraQueryParameters) {
            var _this = this;
            ...
            return new Promise(function (resolve, reject) {
                ...
                _this.authorityInstance.ResolveEndpointsAsync().then(function () {
                    ...
                    var urlNavigate = authenticationRequest.createNavigateUrl(scopes) + "&prompt=" + promptValue + "&response_mode=fragment";
        ...
        UserAgentApplication.prototype.openWindow = function (urlNavigate, title, interval, instance, resolve, reject) {
            ...
            if (this._openedWindows) {
                for (var w in this._openedWindows) {
                    this._openedWindows[w].close();
                }
            }
        ...
        UserAgentApplication.prototype.openPopup = function (urlNavigate, title, popUpWidth, popUpHeight) {
            try {
                ...
                var popupWindow = window.open(urlNavigate, title, 'width=' + popUpWidth + ',height=' + popUpHeight + ',scrollbars=yes,menubar=yes,status=yes,resizable=yes,directories=false,location=false,left=' + left + ',top=' + top_1);
        ...
        UserAgentApplication.prototype.registerCallback = function (expectedState, scope, resolve, reject) {
            ...
            if (!window.callBackMappedToRenewStates[expectedState]) {
                window.callBackMappedToRenewStates[expectedState] =
                    function (errorDesc, token, error, tokenType, tokens) {
                        ...
                        for (var i = 0; i < window.callBacksMappedToRenewStates[expectedState].length; ++i) {
                            try {
                                if (errorDesc || error) {
                                    ...
                                    window.callBacksMappedToRenewStates[expectedState][i].resolve(tokens);
                                }
                            }
                            catch (e) {
            ...
            if (self.isCallback(hash)) {
                ...
                try {
                    if (tokenReceivedCallback) {
                        tokenReceivedCallback(errorDesc, token, error, tokenType, requestInfo.parameters);
                    }
                }
                catch (err) {
